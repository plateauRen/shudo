...
import { sanitizeHtml } from "@tsdaodao/rich-editor";
import {
...
export class TextCell extends MessageCell {
    ...
    render() {
        const { message, context } = this.props
        const content = message?.remoteExtra?.contentEdit || message.content
        const format = getMessageTextFormat(content)
        const isHtml = format === "html"
        const html = isHtml ? sanitizeHtml(getMessageTextHtml(content)) : ""
        ...
        return <MessageBase message={message} context={context} onBubble={() => {
            ...
            <div className="wk-message-text-content">
                {isHtml ? (
                    <div
                        className="wk-message-text-html"
