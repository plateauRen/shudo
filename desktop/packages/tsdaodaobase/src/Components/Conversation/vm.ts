...
export default class ConversationVM extends ProviderListener {
    ...
    didMount(): void {
        ...
        this.messageListener = (message: Message) => {
            ...
            // Hermes envelopes + silent action callbacks
            rehydrateHermesMessage(message)
            if (shouldHideHermesMessage(message)) {
                return
            }
    ...
    toMessageWraps(messages: Array<Message>): Array<MessageWrap> {
        ...
        if (messages) {
            for (const message of messages) {
                rehydrateHermesMessage(message)
                if (shouldHideHermesMessage(message) || message.isDeleted) {
                    continue
                }
    ...
    async sendMessage(content: MessageContent, channel: Channel): Promise<Message> {
        ...
        // Hermes 按钮回传：发出去但不进会话气泡
        if (content.contentType === MessageContentTypeConst.hermesAction) {
            return message
        }
        const messageWrap = new MessageWrap(message)
