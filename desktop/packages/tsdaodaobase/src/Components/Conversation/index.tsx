...
import { RichComposerPanel, sanitizeHtml } from "@tsdaodao/rich-editor";
...
interface ConversationState {
    richComposerOpen?: boolean
}

export class Conversation extends Component<ConversationProps, ConversationState> implements ConversationContext {
    vm!: ConversationVM
    ...
    constructor(props: any) {
        super(props)
        this.state = {
            richComposerOpen: false,
        }
    }

    openRichComposer = () => {
        this.setState({ richComposerOpen: true })
    }

    closeRichComposer = () => {
        this.setState({ richComposerOpen: false })
    }

    sendHtmlMessage = async (html: string) => {
        const clean = sanitizeHtml(html)
        const content = new HtmlMessageText(clean)
        content.format = "html"
        if (this.vm.currentReplyMessage && this.vm.currentHandlerType !== 2) {
            const reply = new Reply()
            reply.messageID = this.vm.currentReplyMessage.messageID
            reply.messageSeq = this.vm.currentReplyMessage.messageSeq
            reply.fromUID = this.vm.currentReplyMessage.fromUID
            const channelInfo = WKSDK.shared().channelManager.getChannelInfo(
                new Channel(this.vm.currentReplyMessage.fromUID, ChannelTypePerson)
            )
            if (channelInfo) {
                reply.fromName = channelInfo.title
            }
            reply.content = this.vm.currentReplyMessage.content
            content.reply = reply
            this.vm.currentReplyMessage = undefined
        }
                ...
                <div className={classNames("wk-conversation", vm.fileDragEnter ? "wk-conversation-dragover" : undefined, vm.currentReplyMessage ? "wk-conversation-hasreply" : undefined)} style={{ "background": chatBg ? `url(${chatBg}) rgb(245, 247, 249)` : undefined }}>
                    ...
                    <div className="wk-conversation-footer">
                        <div className="wk-conversation-footer-content">
                            ...
                            }} toolbar={this.chatToolbarUI()} context={this}
                                onOpenRichComposer={this.openRichComposer}
                    ...
                    {this.state.richComposerOpen
                        ? ReactDOM.createPortal(
                            <RichComposerPanel
                                visible
                                onClose={this.closeRichComposer}
                                onSend={this.sendHtmlMessage}
                            />,
                            document.querySelector(".wk-chat-content-right") ||
                                document.body
                          )
                        : null}
                </div>
