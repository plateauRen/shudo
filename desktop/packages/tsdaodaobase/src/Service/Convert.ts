...
export class Convert {
    ...
    static toMessage(msgMap: any): Message {
        ...
        // Hermes button callbacks are silent (align with iOS store intercept)
        if (contentType === MessageContentTypeConst.hermesAction) {
    ...
    static userToChannelInfo(data: any): ChannelInfo {
        ...
        } else if (data.robot === 1 || data.robot === true) { // Bot
            channelInfo.orgData.identityIcon = "./identity_icon/bot.png"
            channelInfo.orgData.identitySize = { width: "32px", height: "16px" }
        }
