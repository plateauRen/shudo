...
-(NSArray<WKMessage*>*) recvPacketsToMessages:(NSArray<WKRecvPacket*>*)packets {
    ...
    for (WKRecvPacket *packet in packets) {
        ...
        // Hermes envelopes are decoded in-memory to 2100x; persist the unwrapped JSON
        // so DB reload does not keep a type=1 blob under content_type=2100x.
        NSInteger ct = contentType.integerValue;
        if (ct == WKHermesCardContentType || ct == WKHermesActionContentType || ct == WKHermesTableContentType) {
            NSData *encoded = [messageContent encode];
            message.contentData = encoded.length ? encoded : planPayloadData;
        } else {
...
-(WKMessageContent*) decodeContent:(NSData*) contentData contentType:(NSNumber**)contentType{
    ...
    // Robot text envelope → Hermes card (type 21000)
    NSInteger unwrappedType = 0;
    NSDictionary *payload = contentDict;
    if ([WKHermesPayload unwrapCardIfNeeded:&payload contentType:&unwrappedType]) {
        contentDict = payload;
        NSData *rewritten = [NSJSONSerialization dataWithJSONObject:contentDict options:kNilOptions error:nil];
        if (rewritten) {
            contentData = rewritten;
        }
    }
     NSNumber *actContentType = unwrappedType > 0 ? @(unwrappedType) : [contentDict objectForKey:@"type"];
    WKMessageContent *messageContent;
    if(!actContentType) {
