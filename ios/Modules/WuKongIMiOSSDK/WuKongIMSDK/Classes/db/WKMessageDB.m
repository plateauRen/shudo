...
-(WKMessage*) toMessage:(NSDictionary*)dict db:(FMDatabase*)db{
    ...
    if(dict[@"content"] && [dict[@"content"] isKindOfClass:[NSData class]]) {
        NSData *contentData = dict[@"content"];
        // Robot envelope stored as type=1 → unwrap to Hermes card before cell lookup
        [WKHermesPayload unwrapCardDataIfNeeded:&contentData contentType:&contentType];
        message.contentType = contentType;
        message.content = [self decodeContent:contentType data:contentData db:db];
        message.contentData = contentData;
    }else {
        message.contentType = contentType;
