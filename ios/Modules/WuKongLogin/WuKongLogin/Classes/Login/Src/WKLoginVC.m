...
-(void) loadView {
    ...
    self.loginView.onLogin = ^(NSString * _Nonnull mobile, NSString * _Nonnull password,NSString *country) {
        ...
        // Email: send as-is (server matches user.email). Phone: zone + phone.
        NSString *account = [mobile stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet];
        NSString *username = [account containsString:@"@"]
            ? account
            : [NSString stringWithFormat:@"%@%@", country ?: @"", account ?: @""];
