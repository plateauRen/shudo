...
-(void) setupUI {
    self.icon = [[WKImageView alloc] initWithFrame:CGRectMake(0, 0, 44.0f, 44.0f)];
    self.icon.layer.masksToBounds = YES;
    self.icon.layer.cornerRadius = 22.0f;
...
-(void)refresh:(WKContactsHeaderItem*)model {
    ...
    if (model.symbolName.length > 0) {
        self.icon.image = [WKBrandIconHelper circleSymbol:model.symbolName size:44.0f];
    } else if( model.icon && ([model.icon hasPrefix:@"http"] || [ model.icon hasPrefix:@"https"])) {
        [self.icon loadImage:[NSURL URLWithString:model.icon]];
    }else {
        UIImage *asset = [[WKApp shared] loadImage:model.icon moduleID:model.moduleID];
        // Prefer brand circle + tinted glyph when asset is a template-like icon.
        if (asset) {
            self.icon.image = [WKBrandIconHelper circleTemplateImage:[asset imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate] size:44.0f];
        } else {
            self.icon.image = asset;
        }
    }
