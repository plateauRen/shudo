...
-(WKMessage*) sendHtmlMessage:(NSString*)html {
    if (!html.length) {
        return nil;
    }
    ...
    // Basic strip of script/on* for defense in depth (TipTap already sanitizes).
    ...
    WKTextContent *content = [[WKTextContent alloc] initWithContent:safe];
    content.format = @"html";
...
//
    ...
    // ContextController expects Texture's ContextGesture; UILongPress must not be cast (crash).
    ContextGesture *contextGesture = nil;
    if ([gestureRecognizer isKindOfClass:[ContextGesture class]]) {
        contextGesture = (ContextGesture *)gestureRecognizer;
    }
    WKMessageContextController *messageContextController = [[WKMessageContextController alloc] initWithMessage:messageCell.messageModel context:self menusItems:toolbarMenus gesture:contextGesture];
