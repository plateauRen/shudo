...
//    [self setBackgroundColor:[UIColor redColor]];
    ...
    [self.infoBoxBtn addSubview:self.botTag];
...
- (void)layoutSubviews {
    ...
    CGFloat botReserve = self.botTag.hidden ? 0.0f : (self.botTag.lim_width + 4.0f);
    CGFloat maxTitleW = self.infoBoxBtn.lim_width - self.avatarImgView.lim_right - avatarRightSpace - titleRightSpace - botReserve;

    self.titleLbl.lim_left = self.avatarImgView.lim_right + avatarRightSpace;
    [self.titleLbl sizeToFit];
    if (self.titleLbl.lim_width > maxTitleW) {
        self.titleLbl.lim_width = maxTitleW;
    }
    ...
    if (!self.botTag.hidden) {
        self.botTag.lim_left = self.titleLbl.lim_right + 4.0f;
...
- (void)setChannelInfo:(WKChannelInfo *)channelInfo {
    ...
    if(channelInfo && (channelInfo.channel.channelType == WK_PERSON || channelInfo.channel.channelType == WK_CustomerService)) {
        ...
        if(onlineTip) {
            ...
            self.subtitleLbl.text = LLang(@"Bot");
        }else {
            self.subtitleLbl.hidden = YES;
        }
        [self.subtitleLbl sizeToFit];
        self.avatarImgView.uid = channelInfo.channel.channelId;
...
- (WKBotTag *)botTag {
    if (!_botTag) {
        ...
        _botTag.hidden = YES;
        _botTag.userInteractionEnabled = NO;
    }
    return _botTag;
}
