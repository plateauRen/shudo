...
-(void) addDelegates {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(onTranslateDidUpdate:)
                                                 name:WKTranslateDidUpdateNotification
                                               object:nil];
...
-(void) removeDelegates {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:WKTranslateDidUpdateNotification object:nil];

}

- (void)onTranslateDidUpdate:(NSNotification *)note {
    NSString *clientMsgNo = note.userInfo[WKTranslateUserInfoClientMsgNoKey];
    if (!clientMsgNo.length || !self.dataProvider) {
        return;
    }
    NSIndexPath *indexPath = [self.dataProvider indexPathAtClientMsgNo:clientMsgNo];
    if (!indexPath) {
        return;
    }
    WKMessageModel *model = [self.dataProvider messageAtIndexPath:indexPath];
    if (!model) {
        return;
    }
    // Height changes when translation appears — full row reload.
...
-(BOOL) needHandle:(WKMessage*)message {
    ...
    if(message.contentType == WK_HERMES_ACTION) { // Hermes 按钮回传：静默，不进气泡列表
        return false;
    }
