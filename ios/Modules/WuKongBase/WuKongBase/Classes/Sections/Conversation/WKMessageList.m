...
-(BOOL) shouldHideHermesActionMessage:(WKMessageModel*)model {
    if (model.contentType == WK_HERMES_ACTION) {
        return YES;
    }
    if (model.contentType == WK_TEXT && [model.content isKindOfClass:[WKTextContent class]]) {
...
-(void) insertMessage:(WKMessageModel*)model {
    if ([self shouldHideHermesActionMessage:model]) {
        return;
    }
...
-(void) addMessage:(WKMessageModel*)message {
    if ([self shouldHideHermesActionMessage:message]) {
        return;
    }
