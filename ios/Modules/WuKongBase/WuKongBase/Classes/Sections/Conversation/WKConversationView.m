...
// ---------- Hermes 音频顶栏 ----------
@property(nonatomic,strong) WKHermesAudioBar *hermesAudioBar;
...
-(void) setupUI {
    ...
    [self addSubview:self.hermesAudioBar];
...
- (void)dealloc {
    ...
    [self.hermesAudioBar stopAndHide];
...
-(void) addDelegates {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onHermesAudioPlay:) name:WKHermesAudioPlayNotification object:nil];
}
...
-(void) removeDelegates {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:WKHermesAudioPlayNotification object:nil];
}

-(void) onHermesAudioPlay:(NSNotification *)note {
    ...
    NSInteger durationMs = [info[@"duration_ms"] integerValue];
    if (!url.length) return;
    [self.hermesAudioBar playURL:url title:title durationMs:durationMs];
    [self layoutSubviews];
}
...
- (void)layoutSubviews {
    ...
    // Top overlays (topView / Hermes audio bar) shrink messageListView, but the
    // inner table keeps its original height — same as topView, those heights must
    // be included in adjustTableWithOffset or the last bubbles sit under the input.
    CGFloat topOverlay = 0.0f;
    if (!self.topView.hidden) {
        topOverlay += self.topView.lim_height;
    }
    if (self.hermesAudioBar && [self.hermesAudioBar isVisible]) {
        topOverlay += [WKHermesAudioBar preferredHeight];
    }
    ...
    if(otherPanel) {
        ...
        [self.messageListView adjustTableWithOffset:otherPanel.lim_height + topOverlay];
        ...
        if(self.tableOffsetY>0) {
            [self.messageListView adjustTableWithOffset:self.tableOffsetY + topOverlay];
        }else {
            [self.messageListView adjustTableWithOffset:self.input.lim_height + topOverlay];
        }
    ...
    if(!self.topView.hidden) {
        self.topView.lim_top = 0.0f;
        topInset += self.topView.lim_height;
    }
    if (self.hermesAudioBar && [self.hermesAudioBar isVisible]) {
        CGFloat barH = [WKHermesAudioBar preferredHeight];
        self.hermesAudioBar.frame = CGRectMake(0, topInset, self.lim_width, barH);
        [self bringSubviewToFront:self.hermesAudioBar];
        topInset += barH;
    } else if (self.hermesAudioBar) {
        self.hermesAudioBar.frame = CGRectMake(0, 0, self.lim_width, [WKHermesAudioBar preferredHeight]);
    }
    self.messageListView.lim_top = topInset;
    self.messageListView.lim_height = self.lim_height - topInset;
//   
...
- (WKHermesAudioBar *)hermesAudioBar {
    if (!_hermesAudioBar) {
        _hermesAudioBar = [[WKHermesAudioBar alloc] initWithFrame:CGRectMake(0, 0, self.lim_width, [WKHermesAudioBar preferredHeight])];
        __weak typeof(self) weakSelf = self;
        _hermesAudioBar.onVisibilityChange = ^(BOOL visible, CGFloat height) {
            (void)visible;
            (void)height;
            [weakSelf layoutSubviews];
        };
        [WKLiquidGlassHelper attachScrollEdgeInteractionToView:_hermesAudioBar
                                                    scrollView:self.messageListView.tableView
                                                          edge:UIRectEdgeTop];
    }
    return _hermesAudioBar;
}
...
//        [_input setBackgroundColor: ColorSessionMessageInputBar];
        [WKLiquidGlassHelper attachScrollEdgeInteractionToView:_input
                                                    scrollView:self.messageListView.tableView
