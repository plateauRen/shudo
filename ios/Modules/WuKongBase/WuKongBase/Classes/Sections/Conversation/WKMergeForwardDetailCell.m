...
- (void)refresh:(WKMergeForwardDetailModel *)model {
    ...
    UIImage *brand = [WKBrandIconHelper systemAvatarForUID:model.message.fromUid size:self.avatarImgView.lim_width > 0 ? self.avatarImgView.lim_width : 40.0f];
    if (brand) {
    ...
    if([model.message.fromUid isEqualToString:[WKApp shared].config.systemUID]) {
        self.nameLbl.text = [@"系统通知" Localized:self];
    } else if([model.message.fromUid isEqualToString:[WKApp shared].config.fileHelperUID]) {
        self.nameLbl.text = [@"文件传输助手" Localized:self];
    } else if(model.message.from) {
