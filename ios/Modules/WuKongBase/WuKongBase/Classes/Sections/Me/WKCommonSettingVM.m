        ...
        return  @{
            ...
            @"items":@[
                @{
                    ...
                    @"value": themeDesc,
                    @"onClick":^{
                        WKThemeSettingVC *vc = [WKThemeSettingVC new];
...
//    } category:WKPOINT_CATEGORY_COMMONSETTING sort:79000];
    ...
    // 翻译
    [[WKApp shared] setMethod:@"commonsetting.translate" handler:^id _Nullable(id  _Nonnull param) {
        WKTranslateManager *mgr = [WKTranslateManager shared];
        NSString *value = mgr.autoTranslateEnabled
            ? [NSString stringWithFormat:@"%@ · %@", LLang(@"开"), [mgr displayNameForLanguage:mgr.targetLanguage]]
            : LLang(@"关");
        return @{
            @"height": WKSectionHeight,
            @"items": @[
                @{
                    ...
                    @"value": value,
                    @"onClick": ^{
                        WKTranslateSettingVC *vc = [WKTranslateSettingVC new];
                        [[WKNavigationManager shared] pushViewController:vc animated:YES];
                    },
                },
            ],
