...
-(UIView*) memberAvatarView:(CGSize)size {
    ...
    avatarView.uid = self.channel.channelId;
    ...
    if([self.channel.channelId isEqualToString:[WKApp shared].config.systemUID]) {
        nameLbl.text = LLang(@"系统通知");
    } else if([self.channel.channelId isEqualToString:[WKApp shared].config.fileHelperUID]) {
        nameLbl.text = LLang(@"文件传输助手");
    }
