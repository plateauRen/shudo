...
- (void)refresh:(WKMessageModel *)model {
    ...
    UIImage *brand = [WKBrandIconHelper systemAvatarForUID:content.uid size:48.0f];
    if (brand) {
        self.recommendAvatarImgView.image = brand;
    } else if(!content.avatar || [content.avatar isEqualToString:@""]) { // 头像
    ...
    if ([content.uid isEqualToString:[WKApp shared].config.fileHelperUID]) {
        self.recommendNameLbl.text = LLang(@"文件传输助手");
    } else if ([content.uid isEqualToString:[WKApp shared].config.systemUID]) {
        self.recommendNameLbl.text = LLang(@"系统通知");
    } else {
