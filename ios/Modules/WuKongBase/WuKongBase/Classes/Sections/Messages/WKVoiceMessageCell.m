...
-(void) setSendStyle {
    // Pale Feishu send bubble → dark/gray waveform, theme play affordance
    self.highlightedWaveformView.tintColor = [WKApp shared].config.themeColor;
    self.audioWaveformView.tintColor = [[WKApp shared].config.defaultTextColor colorWithAlphaComponent:0.28f];
    self.playIconImgView.tintColor = [UIColor whiteColor];
    [self.playBtnBoxView setBackgroundColor:[WKApp shared].config.themeColor];
    self.durationLabel.textColor = [WKApp shared].config.messageTipColor;
}
