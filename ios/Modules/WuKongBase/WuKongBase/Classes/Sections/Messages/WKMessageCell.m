...
- (void)refresh:(WKMessageModel *)model {
    ...
    if(self.messageModel.isSend) {
        self.bubbleBackgroundView.tintColor = WKApp.shared.config.messageSendBubbleColor;
    }else{
...
- (void)setContentGesturesEnabled:(BOOL)enabled {
    self.mainContainerNode.isGestureEnabled = enabled;
    // Texture 节点默认吃掉整格触摸；关闭后触摸可落到 contentView 上的自定义按钮
    self.mainContainerNode.view.userInteractionEnabled = enabled;
    self.mainContextSourceNode.view.userInteractionEnabled = enabled;
    [self.tapLongTapOrDoubleTapGestureRecognizerWrap.gesture setEnabled:enabled];
}

- (void)setTapGestureWrapEnabled:(BOOL)enabled {
    [self.tapLongTapOrDoubleTapGestureRecognizerWrap.gesture setEnabled:enabled];
}
