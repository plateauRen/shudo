...
#define translateFontSize 13.0f
#define translateTopSpace 6.0f
...
// ---------- 翻译 ----------
@property(nonatomic,strong) UILabel *translateLbl;
@property(nonatomic,strong) UIView *translateSplit;
...
+ (CGSize)translationSizeForMessage:(WKMessageModel *)model maxWidth:(CGFloat)maxWidth {
    NSString *text = [[WKTranslateManager shared] translationAttachedToMessage:model];
    if (!text.length || [[WKTranslateManager shared] isTranslationHiddenForMessage:model]) {
        return CGSizeZero;
    }
    if ([text isEqualToString:[[WKTranslateManager shared] plainTextFromMessage:model]]) {
        return CGSizeZero;
    }
    UIFont *font = [[WKApp shared].config appFontOfSize:translateFontSize];
...
+ (CGSize)contentSizeForMessage:(WKMessageModel *)model {
    ...
    CGSize translationSize = [self translationSizeForMessage:model maxWidth:[WKApp shared].config.messageContentMaxWidth];
    if (translationSize.height > 0) {
        size.height += translateTopSpace + 0.5f + 4.0f + translationSize.height;
        size.width = MAX(size.width, translationSize.width);
    }
...
//    [self.textLbl setTextColor:[WKApp shared].config.defaultTextColor];
    ...
    self.translateSplit = [[UIView alloc] init];
    self.translateSplit.hidden = YES;
    [self.messageContentView addSubview:self.translateSplit];

    self.translateLbl = [[UILabel alloc] init];
    self.translateLbl.numberOfLines = 0;
    self.translateLbl.hidden = YES;
    self.translateLbl.font = [[WKApp shared].config appFontOfSize:translateFontSize];
    [self.messageContentView addSubview:self.translateLbl];
...
+(NSString*) sanitizedHTML:(NSString*)html {
    if (!html.length) return @"";
    NSMutableString *safe = [html mutableCopy];
    NSRegularExpression *scriptRe = [NSRegularExpression regularExpressionWithPattern:@"<script[\\s\\S]*?</script>"
                                                                              options:NSRegularExpressionCaseInsensitive
                                                                                error:nil];
    [scriptRe replaceMatchesInString:safe options:0 range:NSMakeRange(0, safe.length) withTemplate:@""];
    NSRegularExpression *onRe = [NSRegularExpression regularExpressionWithPattern:@"\\son\\w+\\s*=\\s*(\"[^\"]*\"|'[^']*'|[^\\s>]+)"
                                                                          options:NSRegularExpressionCaseInsensitive
                                                                            error:nil];
    [onRe replaceMatchesInString:safe options:0 range:NSMakeRange(0, safe.length) withTemplate:@""];
    return safe;
}
...
+(NSMutableAttributedString*) getContentAttrStr:(WKMessageModel*)message {
    ...
    // WYSIWYG HTML messages — use system HTML renderer (not plain lim_render).
    if (textContent.format && [textContent.format isEqualToString:@"html"]) {
        return [self parseText:textContent isSend:message.isSend parseBefore:nil];
    }
    ...
    if(message.streams && message.streams.count>0) {
        for (WKStream *stream in message.streams) {
            if([stream.content isKindOfClass:WKTextContent.class]) {
                WKTextContent *streamText = (WKTextContent*)stream.content;
                [content appendString:streamText.content ?: @""];
            }
        }
    }
...
+(NSMutableAttributedString*) parseText:(WKTextContent*)content isSend:(BOOL)isSend parseBefore:(void(^)(NSMutableAttributedString *attr))parseBeforeBlock{
    ...
    if(content.content) {
        if(content.format && [content.format isEqualToString:@"html"]) {
            ...
            CGFloat fontSize = [WKApp shared].config.messageTextFontSize;
            NSString *safeHTML = [self sanitizedHTML:content.content];
            NSString *temp = [NSString stringWithFormat:
                    @"<style>body{font-size:%.0fpx;color:%@;font-family:-apple-system;}"
                    @"pre,code{font-family:ui-monospace,Menlo,monospace;font-size:13px;}"
                    @"pre{background:rgba(0,0,0,0.06);padding:8px;border-radius:6px;white-space:pre-wrap;}"
                    @"table{border-collapse:collapse;width:100%%;}"
                    @"th,td{border:1px solid rgba(0,0,0,0.18);padding:4px 8px;}"
                    @"th{background:rgba(0,0,0,0.04);}"
                    @"a{color:#3478f6;}</style>%@",
                    fontSize, [textColor toHexRGB], safeHTML];
            NSAttributedString *htmlAttr = [[NSAttributedString alloc] initWithData:[temp dataUsingEncoding:NSUTF8StringEncoding]
                                                                            options:@{NSDocumentTypeDocumentAttribute:NSHTMLTextDocumentType,
                                                                                      NSCharacterEncodingDocumentAttribute:@(NSUTF8StringEncoding)}
                                                                 documentAttributes:nil
                                                                              error:nil];
            if (htmlAttr) {
                [attrStr appendAttributedString:htmlAttr];
            }
        }else {
...
//    NSString *text = textContent.content;
    ...
    [self refreshTranslationUI:model];
}

- (void)refreshTranslationUI:(WKMessageModel *)model {
    WKTranslateManager *mgr = [WKTranslateManager shared];
    NSString *translated = [mgr translationAttachedToMessage:model];
    BOOL hidden = [mgr isTranslationHiddenForMessage:model];
    BOOL sameAsOriginal = translated.length && [translated isEqualToString:[mgr plainTextFromMessage:model]];
    BOOL show = translated.length > 0 && !hidden && !sameAsOriginal;

    self.translateLbl.hidden = !show;
    self.translateSplit.hidden = !show;
    if (show) {
        self.translateLbl.text = translated;
        UIColor *tip = model.isSend ? [WKApp shared].config.messageTipColor : [WKApp shared].config.tipColor;
        self.translateLbl.textColor = tip;
        self.translateSplit.backgroundColor = [tip colorWithAlphaComponent:0.25f];
        CGSize sz = [[self class] translationSizeForMessage:model maxWidth:MAX(self.messageContentView.lim_width, [WKApp shared].config.messageContentMaxWidth)];
        self.translateLbl.lim_size = sz;
    }

    if ([mgr shouldAutoTranslateMessage:model] && !translated.length && ![model.tmpObject[@"wk_translation_loading"] boolValue]) {
        [mgr translateMessage:model force:NO completion:nil];
    }
}
...
- (void)tapLongTapOrDoubleTapGesture:(TapLongTapOrDoubleTapGestureRecognizerWrap *)recognizer {
    if (recognizer.tapAction == WKTapLongTapOrDoubleTapGestureDoubleTap) {
        if ([self shouldOpenDetailAtPoint:recognizer.tapPoint]) {
            [self openBubbleDetail];
            return;
        }
    }
...
- (WKTapLongTapOrDoubleTapGestureRecognizerEvent *)tapActionAtPoint:(CGPoint)point {
    if ([self pointInsideMessageBody:point] && ![self replyAtPoint:point]) {
        // Wait for possible second tap → bubble detail (single tap still fires after short delay).
        return [WKTapLongTapOrDoubleTapGestureRecognizerEvent action:WKTapLongTapOrDoubleTapGestureRecognizerActionWaitForDoubleTap];
    }
    return [super tapActionAtPoint:point];
}

- (BOOL)pointInsideMessageBody:(CGPoint)point {
    if (!self.messageContentView.superview) return NO;
    CGRect rect = [self.contentView convertRect:self.messageContentView.bounds fromView:self.messageContentView];
    return CGRectContainsPoint(rect, point);
}

- (BOOL)shouldOpenDetailAtPoint:(CGPoint)point {
    if (![self pointInsideMessageBody:point] || [self replyAtPoint:point]) {
        return NO;
    }
    // Don't hijack link / @mention double-taps.
    if (self.textLbl) {
        CGPoint p = [self.textLbl convertPoint:point fromView:self.contentView];
        id<WKMatchToken> token = [self.textLbl matchDidTapAttributedTextInLabelWithPoint:p];
        if (token && (token.type == WKatchTokenTypeLink || token.type == WKatchTokenTypeLink2 || token.type == WKatchTokenTypeMetion)) {
            return NO;
        }
    }
    return YES;
...
- (void)openBubbleDetail {
    NSString *text = @"";
    if ([self.messageModel.content isKindOfClass:[WKTextContent class]]) {
        text = ((WKTextContent *)self.messageModel.content).content ?: @"";
    }
    if (!text.length) return;
    WKBubbleMessageDetailVC *vc = [WKBubbleMessageDetailVC detailWithTitle:@"消息" selectableText:text];
    [[WKNavigationManager shared] pushViewController:vc animated:YES];
}

-(BOOL) replyAtPoint:(CGPoint)point {
    if (self.replyBox.hidden || !self.replyBox.superview) {
        return NO;
    }
    CGRect rectInContentView = [self.contentView convertRect:self.replyBox.bounds fromView:self.replyBox];
...
-(void) layoutSubviews {
    ...
    if (!self.translateLbl.hidden) {
        self.translateSplit.lim_left = 0.0f;
        self.translateSplit.lim_top = self.textLbl.lim_bottom + translateTopSpace;
        self.translateSplit.lim_width = self.messageContentView.lim_width;
        self.translateSplit.lim_height = 0.5f;
        self.translateLbl.lim_left = 0.0f;
        self.translateLbl.lim_top = self.translateSplit.lim_bottom + 4.0f;
        self.translateLbl.lim_width = self.messageContentView.lim_width;
        [self.translateLbl sizeToFit];
        self.translateLbl.lim_width = self.messageContentView.lim_width;
    }
