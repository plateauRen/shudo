...
- (void)viewDidLoad {
    ...
    [self.userHeader addSubview:self.botTag];
...
-(void) refreshData {
    ...
    BOOL isBot = self.viewModel.channelInfo.robot;
...
-(void) layoutUI {
    ...
    CGFloat nameRightInHeader = self.nameLbl.lim_right + 5.0f + self.userAvatarView.lim_right + textToAvatarLeftSpace;
    if (!self.botTag.hidden) {
        self.botTag.lim_left = nameRightInHeader;
        self.botTag.lim_top = self.nameLbl.lim_top + (self.nameLbl.lim_height / 2.0f - self.botTag.lim_height / 2.0f);
        nameRightInHeader = self.botTag.lim_right + 5.0f;
    }
    self.sexImgView.lim_left = nameRightInHeader;
