...
//    [self.navigationController.navigationBar setTranslucent:NO];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(brandThemeChange) name:WKNOTIFY_BRAND_THEME_CHANGE object:nil];
}
...
-(void) brandThemeChange {
    [self viewConfigChange:WKViewConfigChangeTypeBrandTheme];
}
...
-(void) viewConfigChange:(WKViewConfigChangeType)type {
    if(type == WKViewConfigChangeTypeStyle || type == WKViewConfigChangeTypeBrandTheme) {
        ...
        if (self.finishBtn) {
            self.finishBtn.backgroundColor = [WKApp shared].config.themeColor;
        }
    }
