...
-(BOOL) setting:(NSString*)key {
    ...
    if(settingDict && settingDict[key] != nil) {
        return [settingDict[key] boolValue];
    }
    // 服务端尚未下发时：默认打开新消息通知相关开关
    if([key isEqualToString:@"new_msg_notice"] ||
       [key isEqualToString:@"msg_show_detail"] ||
       [key isEqualToString:@"voice_on"] ||
       [key isEqualToString:@"shock_on"]) {
        return YES;
    }
    return false;
}
