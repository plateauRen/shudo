...
@property(nonatomic,strong) WKBotTag *botTag; // Bot 标识
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        // Bot 标识
        [self.contextContainerView addSubview:self.botTag];
        // 自动删除图标
...
-(void) refreshAvatar:(WKConversationWrapModel*)model {
    BOOL hasChannelInfo  = model.channelInfo?true:false;
    CGFloat avatarSize = self.avatarImgView.lim_width > 0 ? self.avatarImgView.lim_width : 56.0f;
    UIImage *brand = [WKBrandIconHelper systemAvatarForUID:model.channel.channelId size:avatarSize];
    if (brand) {
        self.avatarImgView.uid = model.channel.channelId;
...
-(void) refreshBotTag:(WKConversationWrapModel*)model {
    BOOL show = model.channelInfo && model.channelInfo.robot;
    self.botTag.hidden = !show;
    if (show) {
...
-(NSMutableAttributedString*) getLastContent:(WKConversationWrapModel*)model{
    ...
    if(model.channel.channelType == WK_GROUP) { // 群组
        if([self showFromName:model] && !hasDraft) {
            ...
            fullContentStr = [NSString stringWithFormat:@"%@%@: %@",reminderStr,fromName,content];
        }else {
    ...
    NSString *cacheKey = [NSString stringWithFormat:@"%@|%@|%@|%@",
                          model.lastClientMsgNo ?: @"",
                          fullContentStr ?: @"",
                          reminderStr ?: @"",
                          fromName];
    if (model.cachedLastContentAttr && [model.cachedLastContentKey isEqualToString:cacheKey]) {
        return [model.cachedLastContentAttr mutableCopy];
    }
    ...
    if(reminderStr.length>0) {
        [contentAttrStr addAttribute:NSForegroundColorAttributeName value:[WKApp shared].config.themeColor range:[fullContentStr rangeOfString:reminderStr]];
    }
    model.cachedLastContentKey = cacheKey;
    model.cachedLastContentAttr = contentAttrStr;
...
-(void) layoutSubviews {
   ...
   self.onlineBadgeView.lim_top = self.avatarImgView.lim_bottom - self.onlineBadgeView.lim_height;
    ...
    CGFloat tagReserve = 0.0f;
    if (!self.botTag.hidden) {
        tagReserve += self.botTag.lim_width + 4.0f;
    }
    if (!self.officialTag.hidden) {
        tagReserve += self.officialTag.lim_width + 4.0f;
    }
    if (tagReserve > 0 && titleMaxWidth > tagReserve) {
        titleMaxWidth -= tagReserve;
    }
    [self.titleLbl sizeToFit];
    ...
    // Bot 标紧跟昵称；官方标在 Bot 之后（可并存）
    CGFloat tagLeft = self.titleLbl.lim_right + 4.0f;
    if (!self.botTag.hidden) {
        self.botTag.lim_left = tagLeft;
        self.botTag.lim_top = self.titleLbl.lim_top + (self.titleLbl.lim_height / 2.0f - self.botTag.lim_height / 2.0f);
        tagLeft = self.botTag.lim_right + 4.0f;
    }
    self.officialTag.lim_left = tagLeft;
