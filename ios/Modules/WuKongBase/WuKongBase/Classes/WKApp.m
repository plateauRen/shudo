...
//    [self.messageRegitry registerCellClass:[WKSystemMessageCell class] forMessageContentClass:[WKSystemContent class]]; // 系统消息
    ...
    [self.messageRegitry registerCellClass:[WKHermesCardCell class] forMessageContentClass:[WKHermesCardContent class]]; // Hermes 交互卡
    [self.messageRegitry registerCellClass:[WKHermesActionCell class] forMessageContentClass:[WKHermesActionContent class]]; // Hermes 按钮回传
    [self.messageRegitry registerCellClass:[WKHermesTableCell class] forMessageContentClass:[WKHermesTableContent class]]; // Hermes 表格
    [self.messageRegitry registerCellClass:[WKHermesAudioCell class] forMessageContentClass:[WKHermesAudioContent class]]; // Hermes 音频播报
...
- (void)appDidBecomeActive:(NSNotification *)notification  {
    ...
    if([WKApp shared].config.darkModeWithSystem) {
        if (@available(iOS 13.0, *)) {
            // Keep Unspecified so night/day tracks the system; read from screen traits.
            [[WKApp shared].config applyWindowInterfaceStyle];
...
-(void) initPointMethods {
    ...
    // 输入框右侧「展开」图标（飞书样式）→ 富文本编辑窗
    [self setMethod:@"conversation_input_textview_rightview.rich_expand" handler:^id _Nullable(id  _Nonnull param) {
        id<WKConversationContext> context = param[@"context"];
        return [WKRichExpandButton buttonWithContext:context];
...
//    } category:WKPOINT_CATEGORY_PANELCONTENT sort:1000];
    ...
    // 查看（气泡详情页，支持系统选字复制）
    [self setMethod:WKPOINT_LONGMENUS_VIEW handler:^id _Nullable(id  _Nonnull param) {
        WKMessageModel *message = param[@"message"];
        NSInteger type = message.contentType;
        if (type != WK_TEXT && type != WK_HERMES_TABLE && type != WK_HERMES_CARD) {
            return nil;
        }
        ...
        return [WKMessageLongMenusItem initWithTitle:LLangW(@"查看", weakSelf) icon:icon onTap:^(id<WKConversationContext> context){
            WKBubbleMessageDetailVC *vc = nil;
            if (type == WK_HERMES_TABLE && [message.content isKindOfClass:[WKHermesTableContent class]]) {
                vc = [WKBubbleMessageDetailVC detailWithTableContent:(WKHermesTableContent *)message.content];
            } else if (type == WK_HERMES_CARD && [message.content isKindOfClass:[WKHermesCardContent class]]) {
                WKHermesCardContent *card = (WKHermesCardContent *)message.content;
                NSMutableString *text = [NSMutableString string];
                if (card.title.length) [text appendFormat:@"%@\n", card.title];
                if (card.body.length) [text appendFormat:@"%@\n", card.body];
                if (card.descText.length) [text appendFormat:@"%@\n", card.descText];
                if (card.contentText.length && !text.length) [text appendString:card.contentText];
                vc = [WKBubbleMessageDetailVC detailWithTitle:card.title.length ? card.title : @"Hermes"
                                             selectableText:[text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
            } else if ([message.content isKindOfClass:[WKTextContent class]]) {
                WKTextContent *textContent = (WKTextContent *)message.content;
                vc = [WKBubbleMessageDetailVC detailWithTitle:@"消息" selectableText:textContent.content ?: @""];
            }
            if (vc) {
                [[WKNavigationManager shared] pushViewController:vc animated:YES];
            }
        }];
    } category:WKPOINT_CATEGORY_MESSAGE_LONGMENUS sort:3100];

    // 复制
    [[WKApp shared] addMessageAllowCopy:WK_TEXT];
    [[WKApp shared] addMessageAllowCopy:WK_HERMES_TABLE];
    [[WKApp shared] addMessageAllowCopy:WK_HERMES_CARD];
    [self setMethod:WKPOINT_LONGMENUS_COPY handler:^id _Nullable(id  _Nonnull param) {
        ...
        return [WKMessageLongMenusItem initWithTitle:LLangW(@"复制", weakSelf) icon:icon onTap:^(id<WKConversationContext> context){
            ...
            if ([message.content isKindOfClass:[WKHermesTableContent class]]) {
                ...
                if (card.title.length) [text appendFormat:@"%@\n", card.title];
                if (card.body.length) [text appendFormat:@"%@\n", card.body];
                if (card.descText.length) [text appendString:card.descText];
    ...
    // 翻译 / 显示原文
    [self setMethod:WKPOINT_LONGMENUS_TRANSLATE handler:^id _Nullable(id  _Nonnull param) {
        WKMessageModel *message = param[@"message"];
        if (message.contentType != WK_TEXT) {
            return nil;
        }
        NSString *plain = [[WKTranslateManager shared] plainTextFromMessage:message];
        if (!plain.length) {
            return nil;
        }
        BOOL hasShown = [[WKTranslateManager shared] translationAttachedToMessage:message].length > 0 &&
                        ![[WKTranslateManager shared] isTranslationHiddenForMessage:message];
        NSString *title = hasShown ? LLangW(@"显示原文", weakSelf) : LLangW(@"翻译", weakSelf);
        UIImage *icon = [GenerateImageUtils generateTintedImgWithImage:[weakSelf imageName:@"Conversation/ContextMenu/Copy"] color:weakSelf.config.contextMenu.primaryColor backgroundColor:nil];
        if (@available(iOS 13.0, *)) {
            UIImage *sys = [UIImage systemImageNamed:@"character.book.closed"];
            if (sys) {
                icon = [sys imageWithTintColor:weakSelf.config.contextMenu.primaryColor renderingMode:UIImageRenderingModeAlwaysOriginal];
            }
        }
        return [WKMessageLongMenusItem initWithTitle:title icon:icon onTap:^(id<WKConversationContext> context){
            if (hasShown) {
                [[WKTranslateManager shared] setTranslationHidden:YES forMessage:message];
                [[NSNotificationCenter defaultCenter] postNotificationName:WKTranslateDidUpdateNotification
                                                                    object:nil
                                                                  userInfo:@{WKTranslateUserInfoClientMsgNoKey: message.clientMsgNo ?: @""}];
                return;
            }
            [[WKTranslateManager shared] setTranslationHidden:NO forMessage:message];
            [[WKTranslateManager shared] translateMessage:message force:YES completion:^(NSString *translated, NSError *error) {
                if (error) {
                    [[WKNavigationManager shared].topViewController.view showHUDWithHide:error.localizedDescription ?: LLangW(@"翻译失败", weakSelf)];
                }
            }];
        }];
    } category:WKPOINT_CATEGORY_MESSAGE_LONGMENUS sort:2950];
...
//        }
    ...
    [self setMethod:WKPOINT_ME_WEB handler:^id _Nullable(id  _Nonnull param) {
        UIImage *icon = [WKBrandIconHelper circleSymbol:@"desktopcomputer" size:30.0f];
        return [WKMeItem initWithTitle:LLangW(@"网页端",weakSelf) icon:icon nextSectionHeight:10.0f onClick:^{
    ...
    [self setMethod:WKPOINT_ME_NEWMSGNOTICE handler:^id _Nullable(id  _Nonnull param) {
        UIImage *icon = [WKBrandIconHelper circleSymbol:@"bell.fill" size:30.0f];
        return [WKMeItem initWithTitle:LLangW(@"新消息通知",weakSelf) icon:icon onClick:^{
    ...
    [self setMethod:WKPOINT_ME_INVITE handler:^id _Nullable(id  _Nonnull param) {
        ...
        UIImage *icon = [WKBrandIconHelper circleSymbol:@"qrcode" size:30.0f];
        return [WKMeItem initWithTitle:LLangW(@"我的邀请码",weakSelf) icon:icon onClick:^{
    ...
    [self setMethod:WKPOINT_ME_COMMON handler:^id _Nullable(id  _Nonnull param) {
        UIImage *icon = [WKBrandIconHelper circleSymbol:@"gearshape.fill" size:30.0f];
        return [WKMeItem initWithTitle:LLangW(@"通用",weakSelf) icon:icon onClick:^{
    ...
    // Hermes 按钮回传：仍发出去给机器人，但不进会话列表 / 最近会话摘要
    [[WKSDK shared].chatManager addMessageStoreBeforeIntercept:@"hermes_action" intercept:^BOOL(WKMessage * _Nonnull message) {
        if (message.contentType == WK_HERMES_ACTION) {
            return NO;
        }
        // 历史文本协议回传也隐藏
        if (message.contentType == WK_TEXT && [message.content isKindOfClass:[WKTextContent class]]) {
            NSString *text = [(WKTextContent *)message.content content] ?: @"";
            if ([text containsString:@"::hermes_action::"]) {
                return NO;
            }
        }
        return YES;
