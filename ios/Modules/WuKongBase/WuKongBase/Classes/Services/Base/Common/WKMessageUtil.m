...
+(WKMessageContent*) decodeMessageContent:(NSDictionary*)payloadDict contentType:(NSNumber**)contentType{
    ...
    // Robot API only allows type=1; Hermes cards are embedded as text envelopes.
    ...
    NSNumber *contentTpe = unwrappedType > 0 ? @(unwrappedType) : payloadDict[@"type"];
    if(!contentTpe) {
...
+(WKMessage*) toMessage:(NSDictionary*)messageDict {
   ...
   NSDictionary *headerDict =  messageDict[@"header"];
    ...
    // Persist unwrapped Hermes payload (not the type=1 envelope) for DB reload.
    NSInteger ct = contentType.integerValue;
    if (ct == WKHermesCardContentType || ct == WKHermesActionContentType || ct == WKHermesTableContentType || ct == WKHermesAudioContentType) {
...
+(WKStream*) toStream:(NSDictionary*)streamDict message:(WKMessage*)message{
    ...
    if(blobDict && [blobDict isKindOfClass:[NSDictionary class]]) {
        ...
        if (ct == WKHermesCardContentType || ct == WKHermesActionContentType || ct == WKHermesTableContentType || ct == WKHermesAudioContentType) {
            NSData *encoded = [messageContent encode];
            stream.contentData = encoded.length ? encoded : [NSJSONSerialization dataWithJSONObject:blobDict options:kNilOptions error:nil];
        } else {
